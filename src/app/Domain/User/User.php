...
class User {
    ...
    public function register($username, $password, $moreInfo = array()) {
        ...
        $newUserInfo['upwd'] = $this->encryptPassword($password, $newUserInfo['salt']);
        $newUserInfo['utime'] = $_SERVER['REQUEST_TIME'];
    ...
    public function login($username, $password) {
        $user = $this->getUserByUsername($username, 'uid,upwd,salt');
        if (!$user) {
            return false;
        }
        
        $encryptPassword = $this->encryptPassword($password, $user['salt']);
        if ($encryptPassword !== $user['upwd']) {
            return false;
        }
